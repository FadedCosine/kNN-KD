...
def get_save_knntargets_parser(default_task=None):
    # add by zx, just copy from get_validation_parser
    # and add datastore args
...
def add_knntargets_args(parser):
    group = parser.add_argument_group("knntargets")
    ...
    group.add_argument("--save-k", default=8, type=int)
    ...
    group.add_argument("--store-in-normal-npy", default=False, action='store_true', help="we store knntargets in memmap by default, but if your machine's RAM is larget enouch, you can store the files in normal npy, and directly load them into RAM when training")
    ...
    # for save knn targets
    group.add_argument("--knndistance-fp16", action='store_true',
                       help="if save only fp16 for knn distances")
    group.add_argument("--knntarget-size", metavar="N", default=1, type=int,
                       help="knntargets size")
    group.add_argument("--knntarget-mmap", default=None, type=str, help="save dir for knntargets")
...
def add_knn_record_args(parser):
    ...
    group.add_argument('--knn-record-target', default=False, action='store_true')
